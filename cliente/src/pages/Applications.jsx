...
const Applications = () => {
  ...
  const updateResume = async () => {
    ...
    try {
      ...
      const {data} = await axios.post(backendUrl+'/api/users/upadete-resume', formData, {headers:{Authorization:`Brarer ${token}`}})
        ...
        <div className='flex gap-2 mb-6 mt-3'>
          {
            ...
            ? <>
              <label className=' flex items-center' htmlFor="resumeUpdate">
                <p className=' bg-teal-100 text-teal-600 px-4 py-2 rounded-lg mr-2'>{resume ? resume.name : "Selecionar Currículo"}</p>
                <input id='resumeUpdate' onChange={e => setResume(e.target.files[0])} accept='aplicacao/pdf' type="file" hidden />
                <img src={assets.profile_upload_icon} alt="" />
              </label>

              <button onClick={updateResume} className=' bg-green-100 border-green-400 rounded-lg px-4 py-2'>Salvar</button>
            </>
            : <div className=' flex gap-2'>
              <a target='_blank' href={userData.resume} className='bg-teal-100 text-teal-600 px-4 py-2 rounded-lg' >Currículo</a>
